""" id_project3_characteristics.py
...
#       For details, review the import statements in id_project3_main.py

import pandas as pd
import id_project3_etl as etl
import util
import sys
import numpy as np
...
#           and use it to test if the inputs of id_project3_characteristics are proper
# ----------------------------------------------------------------------------------------
...
def vol_cal(ret, cha_name, ret_freq_use: list):
    ...
    # Extract daily return dataframe from the input dictionary
    daily_ret_df = ret['Daily'].copy()

    # Create an empty dictionary to store the monthly return volatility for stocks
    monthly_vol_dict = {}

    # Loop through each ticker
    for tic in daily_ret_df.columns:
        # Extract the specific ticker and drop if null
        daily_ret_series = daily_ret_df[tic].dropna()

        # Resample daily returns to monthly
        monthly_ret = daily_ret_series.resample('ME')

        # Calculate the standard deviation
        # Create an empty list to store the standard deviation for each month
        monthly_vol = []
        ...
        monthly_vol_ser = pd.Series(monthly_vol, index=monthly_ret.groups.keys())

        # Convert the index to PeriodIndex with year-month frequency
        monthly_vol_ser.index = monthly_vol_ser.index.to_period('M')
        monthly_vol_dict[tic] = monthly_vol_ser

    # Convert the dictionary to a DataFrame
    monthly_vol_df = pd.DataFrame(monthly_vol_dict)

    # Adjust the column names with the cha name
    monthly_vol_df.columns = [f"{tic}_{cha_name}" for tic in monthly_vol_df.columns]

    # Drop rows with all NaN values
    monthly_vol_df.dropna(how='all', inplace=True)

    # Set the index name
    monthly_vol_df.index.name = 'Year_Month'

    return monthly_vol_df
...
def cha_main(ret, cha_name, ret_freq_use: list):
    ...
    ret : dict
        A dictionary containing two items, where each item is a DataFrame that provides daily and monthly returns.
        See the docstring of the `aj_ret_dict` function in id_project3_etl.py for a description of this dictionary.
